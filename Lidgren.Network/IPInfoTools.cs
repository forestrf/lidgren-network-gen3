...
namespace Lidgren.Network
{
    ...
    {
        [FieldOffset(0)] public IntPtr ifu_broadaddr;

        [FieldOffset(0)] public IntPtr ifu_dstaddr;
    }
    ...
    {
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)] public byte[] u6_addr8;
    }
                ...
                while (next != IntPtr.Zero)
                {
                    ...
                    if (addr.ifa_addr != IntPtr.Zero)
                    {
                        ...
                        switch (sockaddr.sin_family)
                        {
                            ...
                            case AF_INET:
                                if (name == networkInterfaceName)
                                {
                                    ...
                                    var ipAddr = new IPAddress(netmask.sin_addr);
                                        // IPAddress to format into default string notation
                            ...
                            {
                                var sockaddrll =
                                    (sockaddr_ll) Marshal.PtrToStructure(addr.ifa_addr, typeof (sockaddr_ll));
                                if (sockaddrll.sll_halen > sockaddrll.sll_addr.Length)
                                {
                                    Console.Error.WriteLine(
                                        "Got a bad hardware address length for an AF_PACKET {0} {1}",
